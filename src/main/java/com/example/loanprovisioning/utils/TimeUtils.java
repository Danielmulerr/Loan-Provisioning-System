...
import com.example.loanprovisioning.dto.IncreaseDateRange;
import lombok.val;
...
public class TimeUtils {
    ...
    public static IncreaseDateRange getIncreaseDateRange(LocalDateTime fromDate, LocalDateTime toDate) {
        if (ObjectUtils.isEmpty(fromDate) || ObjectUtils.isEmpty(toDate)) {
            val firstRangeStart = LocalDateTime.now();
            return new IncreaseDateRange(
                    Timestamp.valueOf(firstRangeStart.minusDays(3)),
                    Timestamp.valueOf(firstRangeStart.minusDays(2)),
                    Timestamp.valueOf(firstRangeStart.minusDays(1)),
                    Timestamp.valueOf(firstRangeStart)
            );
        }
